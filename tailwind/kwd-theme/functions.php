...
function kwd_fonts_url(): string {
	...
	$poppins = _x( 'on', 'Poppins font: on or off', 'kwd' );
	$dm_sans = _x( 'on', 'DM Sans font: on or off', 'kwd' );
...
function kwd_enqueue_scripts(): void {
	...
	wp_register_script( 'kwd-script', get_theme_file_uri( 'assets/js/app.js' ), array(), $theme->get( 'Version' ), true );
	$localize_strings = array(
		'ajax_url'   => admin_url( 'admin-ajax.php' ),
	);
	wp_localize_script( 'kwd-script', 'localizeStrings', $localize_strings );
	wp_enqueue_script( 'kwd-script' );
}
 ...
 * @return array|string
 */
function kwd_nav_menu_add_submenu_class( array $classes, WP_Term $args, $depth ): array|string {
...
function kwd_widgets_init(): void {
...
add_action( 'widgets_init', 'kwd_widgets_init' );
